...
ADD CONSTRAINT uq_cus_prod UNIQUE (cus_id, prod_id);
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM CartItems WHERE cus_id = @cus_id AND prod_id = @prod_id)
    BEGIN
    ...
    BEGIN
        ...
        INSERT INTO CartItems (cus_id, prod_id, quantity)
...
AS
BEGIN
    DELETE FROM CartItems WHERE cus_id = @cus_id AND prod_id = @prod_id;
END;
